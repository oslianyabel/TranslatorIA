...
def audio_time_stretch(input_file: str, output_file: str, factor: float):
    x, sr = librosa.core.load(input_file)
    x_stretched = stretch(x, factor)

    # if out_file is None:
    #     out_file = path.join(path.dirname(args.in_file), 'time_stretched.wav')
    
    soundfile.write(output_file, x_stretched, sr)
